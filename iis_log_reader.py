...
def analyze_logs_with_gpt_separately(logs, max_retries=3):
    ...
    for i, log in enumerate(logs[:6]):
        prompt = (
            "Aşağıda IIS web sunucusundan alınan bir log kaydı var. "
            "Bu logu anomali, saldırı ya da kötüye kullanım açısından analiz et. "
            "Aşağıdaki formatta sadece geçerli JSON olarak çıktı ver:\n"
...
def analyze_iis_logs(log_file_path):
